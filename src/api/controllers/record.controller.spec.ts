...
describe('RecordController', () => {
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            ...
            findByMBID: jest.fn().mockResolvedValue({
              _id: '1',
              name: 'Record 1',
              price: 100,
              qty: 10,
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException when record not found', async () => {
      ...
      await expect(
        recordController.update('nonexistentId', updateRecordDto),
  ...
  describe('findByMBID', () => {
    it('should find a record by MBID', async () => {
      const result = await recordController.findByMBID(
        '11af85e2-c272-4c59-a902-47f75141dc97',
      );

      expect(service.findByMBID).toHaveBeenCalledWith(
        '11af85e2-c272-4c59-a902-47f75141dc97',
      );
      expect(result).toEqual({
        _id: '1',
    ...
    it('should throw NotFoundException when record not found', async () => {
      ...
      await expect(
        recordController.findByMBID('nonexistentId'),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('fetchMusicBrainzData', () => {
    it('should fetch MusicBrainz data', async () => {
      const result = await recordController.fetchMusicBrainzData(
        '11af85e2-c272-4c59-a902-47f75141dc97',
      );

      expect(service.fetchMusicBrainzDataPublic).toHaveBeenCalledWith(
        '11af85e2-c272-4c59-a902-47f75141dc97',
      );
      expect(result).toEqual({
        artist: 'The Cure',
    ...
    it('should throw NotFoundException when MusicBrainz data not found', async () => {
      ...
      await expect(
        recordController.fetchMusicBrainzData('invalidId'),
      ).rejects.toThrow(NotFoundException);
    });
  });
});
